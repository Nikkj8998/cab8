...
export const LeadsList = () => {
  ...
  const fetchLeads = async (filters: {
    ...
    try {
      ...
      if (filters.serviceFilter) params.append('service_interest', filters.serviceFilter);
  ...
  const loadLeads = useCallback(() => {
    fetchLeads({ searchQuery, statusFilter, sourceFilter, ownerFilter, qualityFilter, serviceFilter, quickFilter });
  }, [searchQuery, statusFilter, sourceFilter, ownerFilter, qualityFilter, serviceFilter, quickFilter]);
  ...
  const handleSaveLead = async (isEdit: boolean) => {
    ...
    try {
      const payload = isEdit ? { 
        id: currentLead.id, 
        ...currentLead,
        status: (currentLead.lead_status || 'New').toLowerCase().substring(0, 15),
        ...
        ...currentLead,
        status: (currentLead.lead_status || 'New').toLowerCase().substring(0, 15),
        message: currentLead.questions || '',
        original_message: currentLead.questions || ''
      };
    ...
    <div className="space-y-6">
        ...
        <CardContent className="p-4">
          ...
          {showFilters && (
            <div className="mt-4 pt-4 border-t grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div>
                <Label className="text-xs mb-1 block">Service Interest</Label>
                <Select value={serviceFilter} onValueChange={(v) => { setServiceFilter(v === 'all' ? '' : v); setCurrentPage(1); }}>
                  <SelectTrigger>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gradient-to-r from-slate-800 to-slate-700 border-b-2 border-slate-600">
                <tr>
                  ...
                  <th className="p-4 text-left font-semibold text-white text-xs uppercase tracking-wide">Service Interest</th>
                  ...
                  <th className="p-4 text-left font-semibold text-white text-xs uppercase tracking-wide">Lead Owner</th>
                ...
                {loading ? (
                  ...
                  paginatedLeads.map(lead => (
                    <tr key={lead.id} className="border-b hover:bg-blue-50 cursor-pointer transition-all duration-200 hover:shadow-sm" onClick={() => handleEditLead(lead)}>
                      ...
                      <td className="p-4 text-gray-700">{lead.service_interest || '-'}</td>
                      ...
                      <td className="p-4 text-gray-700">{lead.lead_owner || 'Unassigned'}</td>
                      <td className="p-4">
                        <Badge className={`text-xs font-semibold px-2 py-1 rounded ${sourceColors[lead.source] || 'bg-gradient-to-r from-gray-400 to-gray-500 text-white'}`}>
      ...
      <Dialog open={addDialogOpen || editDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-3xl max-h-[95vh] overflow-y-auto bg-white border-0 shadow-2xl">
          <DialogHeader className="bg-gradient-to-r from-slate-800 to-slate-700 text-white px-6 py-4 -mx-6 -mt-6 mb-2 rounded-t-lg relative">
            ...
            {editDialogOpen && currentLead.created_at && (
              <div className="absolute top-4 right-12 text-sm text-white font-medium">
                {new Date(currentLead.created_at).toLocaleDateString()} {new Date(currentLead.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
              </div>
            )}
          </DialogHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 py-6 px-6">
            ...
            <div className="space-y-2">
              <Label className="text-sm font-semibold text-gray-900">Creation Date</Label>
              <Input
                type="datetime-local"
                value={currentLead.created_at ? new Date(new Date(currentLead.created_at).getTime() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 16) : ''}
                onChange={(e) => setCurrentLead({ ...currentLead, created_at: e.target.value })}
                className="bg-blue-50 border-2 border-blue-200 focus:border-blue-500 focus:bg-white"
              />
            </div>
            <div className="space-y-2">
